# egain-article-fetch
Browser console script to bulk-fetch article IDs and metadata from eGain AI Knowledge Hub folders, including nested subfolders.
eGain Article Fetch
...
How to use it
Open the eGain AI Knowledge Hub in Chrome, on any folder — as long as the folder tree on the left is visible.
Open DevTools (F12, or Cmd+Opt+I / Ctrl+Shift+I) → Console tab.
Paste the entire contents of egain-article-id-fetch.js and press Enter. You'll see a "Ready" message with the available commands.
Run one of:
...
Repeat step 4 for as many folders/branches as you want — results accumulate in the browser tab's sessionStorage across runs, and duplicates (same folder + same article ID) are automatically skipped.
When done, run:
...
Command
What it does
egainFetch.showCollected()
Prints a table preview of everything collected so far
egainFetch.clearCollected()
Wipes collected data so you can start fresh
